...
| Variable                                                             | Purpose                                                             | Consumed by                                                 | Rotation / blast radius                                                                                                                                                                      |
| -------------------------------------------------------------------- | ------------------------------------------------------------------- | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IDENTITY_DATABASE_URL`                                              | Auth Postgres DSN (owns this schema)                                | `internal/config`, `store.Connect`                          | as `AUTH_DATABASE_URL` above                                                                                                                                                                 |
| `IDENTITY_REDIS_URL`                                                 | Cache, rate limits, MFA tickets, OAuth state                        | `cmd/server/main.go`                                        | Unset = MFA + strict limits disabled (dev only). Flush on rotation.                                                                                                                          |
| `IDENTITY_IP_HASH_KEY`                                               | HMAC-SHA256 key pseudonymising IPs (audit, sessions, device alerts) | `internal/iphash`                                           | **Required, 32 bytes b64.** Rotating breaks correlation of historical IPs (by design). Quarterly rotation optional.                                                                          |
| `IDENTITY_ENCRYPTION_KEY`                                            | AES-256-GCM sealbox for TOTP secrets at rest                        | `internal/mfa`                                              | **Required, 32 bytes b64.** ⚠️ Rotating requires re-sealing every enrolled user's MFA secret first — write a migration before rotating; otherwise all MFA logins fail until users re-enroll. |
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| `IDENTITY_ENCRYPTION_KEY` | Treat all MFA seeds exposed → force re-enroll (disable+reenroll), then rotate                                                                        |
